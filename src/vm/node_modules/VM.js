...
function getGatewayIp(gw, vmobj)
{
    ...
    // validLocalRoute() guarantees we have a valid macs[nnn] value
    if (macMatch) {
        nicIdx = macMatch[1];
    } else if (nicMatch) {
        nicIdx = nicMatch[1];
...
function updateRoutes(vmobj, payload, log, callback)
{
        ...
        if (vmobj.routes.hasOwnProperty(key)) {
            ...
            if (payload.hasOwnProperty('remove_routes')
                ...
                routeCmds.push({
                    'cmd': 'delete',
                    'destination': key,
                    'gateway': getGatewayIp(vmobj.routes[key])
                });
    ...
    for (key in payload.set_routes) {
        if (payload.set_routes.hasOwnProperty(key)) {
            ...
            // exists, we are replacing the destination and must first delete
            // the old entry
            if (vmobj.routes.hasOwnProperty(key)) {
                routeCmds.push({
                    'cmd': 'delete',
                    'destination': key,
                    'gateway': getGatewayIp(vmobj.routes[key], vmobj)
                });
            }
            ...
            routeCmds.push({
                    'cmd': 'add',
                    'destination': key,
                    'gateway': getGatewayIp(routes[key], vmobj)
            });
        }
    }
    ...
    vasync.pipeline({
        'funcs': [
            function writeRoutes(_, cb) {
                fs.writeFile(filename, JSON.stringify(routes, null, 2), cb);
            },
            function liveUpdateRoutes(_, cb) {
                // TODO: Check if HVM or not running
                routeCmds.forEach(function runRoute(cmd) {
                    var routeCmd = [ '-z', vmobj.zonename ];

                    routeCmd.push(cmd.cmd);
                    routeCmd.push(cmd.destination);
                    routeCmd.push(cmd.gateway);

                    route(routeCmd, log, function routeCmdCb(err, res) {
                        if (err) {
                            log.debug('could not ' + cmd.cmd + ' '
                                + cmd.destination);
                        } else {
                            log.debug(cmd.cmd + 'ed route ' + cmd.destination);
                        }
                        cb(null, res);
                    });
                });

                // XXX: I dont think we want to stop if one of the route
                // commands fails and leaves things partially done, so
                // for now at least, we'll always return success and just
                // log individual failures.
                return true;
            }
        ]},
        function updateRouteError(err, results) {
            if (results.ndone === 1 && results.nerrors === 0) {
